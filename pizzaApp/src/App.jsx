...
function App() {
  ...
  const handleSortingByName = () => {
    const sortedPizzaNameList = filteredPizza.sort((a, b) =>
      a.name.localeCompare(b.name)
    );
    setFilteredPizza(sortedPizzaNameList);
  };

  const handleSortingByPrice = () => {
    const sortedMaxPriceList = filteredPizza.sort((a, b) => a.price - b.price);
    setFilteredPizza(sortedMaxPriceList);
  };
    ...
    async function loadPizzaById(id) {
      ...
      orderedPizza.map((order) => {
        if (order.id === dataOfOrderedPizza[0].id && deleteOrAdd === "add") {
          ...
          deleteOrAdd === "delete" && order.ordered===true
        ) {
          if (order.amount > 0) {
            order.amount -= 1;
          }
  ...
  return (
    <div className="homepage">
      ...
      <div id="menu" ref={divRef}>
        ...
        <Sorting
          onSort1={() => handleSortingByName()}
          onSort2={() => handleSortingByPrice()}
        ></Sorting>
        <div id="pizza-list">
